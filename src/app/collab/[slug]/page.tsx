...
import { MapPin, Home, Compass, ChevronUp, ChevronDown, Pencil, Users } from "lucide-react";
...
import { useCollabBridge } from "@/hooks/use-collab-bridge";
...
export default function CollabPage() {
  ...
  // Init realtime + bridge to trip-store
  useCollabSession(slug);
  useCollabBridge();
  ...
  if (syncStatus === "offline" && locations.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[50vh] gap-4 text-muted-foreground">
        <Users className="h-8 w-8" />
        <p className="text-lg font-medium">Session not found or expired</p>
        <p className="text-sm">This collaborative session may have expired. Create a new one from your trip.</p>
      </div>
    );
  }
